...
#include <cassert>
#include <array>
#include "FAST_Library.h"  // This header contains the declarations for the external routines


////////////////////// ATA
void fast::OpenFAST::getPlatformDisplacement(double* disp, int nSize) {
    if (nSize < 3) {
        throw std::runtime_error("getPlatformDisplacement: array size must be at least 3");
    }
    // Copy the 3 rigid-body displacement values (surge, sway, heave)
    for (int i = 0; i < 3; i++) {
        disp[i] = platformDisp[i];
    }
}
void fast::OpenFAST::updatePlatformDisplacement() {
    // Temporary storage for the translation displacement (3 components)
    float tempDisp[3] = {0.0f, 0.0f, 0.0f};
    int err = 0;
    char errMsg[INTERFACE_STRING_LENGTH] = {0};
    int iTurb = 0; // Assuming that the platform info is associated with turbine 0.
                   // Adjust if necessary.

    // Call the external Fortran routine to retrieve the platform displacement.
    // The routine should be declared in FAST_Library.h as:
    // EXTERNAL_ROUTINE void FAST_PlatformDisplacement(int * iTurb, float * TranslationDisp, int *ErrStat, char *ErrMsg);
    FAST_PlatformDisplacement(&iTurb, tempDisp, &err, errMsg);

    if (err != 0) {
        throw std::runtime_error("Error in FAST_PlatformDisplacement: " + std::string(errMsg));
    }

    // Update the internal member variable with the retrieved values.
    platformDisp.resize(3);
    for (int i = 0; i < 3; i++) {
        platformDisp[i] = tempDisp[i];
    }
}
///////////////////////////////
