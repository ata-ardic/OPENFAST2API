...
!================================================================================================================================== ATA
subroutine FAST_PlatformDisplacement(iTurb_c, TranslationDisp_c, ErrStat_c, ErrMsg_c) BIND(C, NAME='FAST_PlatformDisplacement')
  use ISO_C_BINDING
  implicit none
  ! Inputs/Outputs for the C binding
  INTEGER(C_INT),         INTENT(IN)  :: iTurb_c
  REAL(C_FLOAT),          INTENT(OUT) :: TranslationDisp_c(3)
  INTEGER(C_INT),         INTENT(OUT) :: ErrStat_c
  CHARACTER(KIND=C_CHAR), INTENT(OUT) :: ErrMsg_c(1025)
  
  INTEGER :: iTurb, i
  CHARACTER(len=1025) :: tempErrMsg

  ! Convert turbine index from C (0-based) to Fortran (1-based)
  iTurb = iTurb_c + 1

  ! Initialize error status and clear error message
  ErrStat_c = 0
  tempErrMsg = ""
  do i = 1, 1025
     ErrMsg_c(i) = c_null_char
  end do

  ! Check that the platform output mesh has been committed.
  if (.NOT. Turbine(iTurb)%ED%y%PlatformPtMesh%Committed) then
     ErrStat_c = 1  ! Or use your defined fatal error code, e.g., ErrID_Fatal
     tempErrMsg = "PlatformPtMesh has not been committed."
     ErrMsg_c = TRANSFER(TRIM(tempErrMsg)//C_NULL_CHAR, ErrMsg_c)
     return
  end if

  ! Retrieve the instantaneous platform translational displacements.
  TranslationDisp_c(1) = REAL(Turbine(iTurb)%ED%y%PlatformPtMesh%TranslationDisp(1,1), C_FLOAT)
  TranslationDisp_c(2) = REAL(Turbine(iTurb)%ED%y%PlatformPtMesh%TranslationDisp(2,1), C_FLOAT)
  TranslationDisp_c(3) = REAL(Turbine(iTurb)%ED%y%PlatformPtMesh%TranslationDisp(3,1), C_FLOAT)

end subroutine FAST_PlatformDisplacement
